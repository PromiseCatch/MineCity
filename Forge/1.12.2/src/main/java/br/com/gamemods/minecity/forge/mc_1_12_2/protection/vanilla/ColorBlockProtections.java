...
import br.com.gamemods.minecity.forge.mc_1_12_2.event.BlockGrowEvent;
import br.com.gamemods.minecity.forge.mc_1_12_2.event.PistonMoveEvent;
import br.com.gamemods.minecity.forge.mc_1_12_2.event.PlayerTeleportDragonEggEvent;
...
public class ColorBlockProtections extends BlockProtections {
    private final MineCityColor mod;

    public ColorBlockProtections(MineCityColor mod) {
        super(mod);
        this.mod = mod;
    }
    ...
    public void onPlayerRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
        ...
        int result = onPlayerRightClickBlock(
                ...
                mod.block(event.getWorld(), event.getPos()),
                ColorUtil.toDirection(event.getFace()),
                true
        );

        if (result == 3) event.setCanceled(true);
        else if (result == 1) {
            event.setUseItem(Event.Result.DENY);
            event.setCanceled(true);
        }
        else if (result == 2) event.setUseBlock(Event.Result.DENY);
    }
